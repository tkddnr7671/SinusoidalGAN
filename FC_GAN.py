...
with g.as_default():
    ...
    # for standard GAN: Loss function in each module: G and D
    result_of_fake = tf.clip_by_value(result_of_fake, eps, 1.0 - eps)
    result_of_real = tf.clip_by_value(result_of_real, eps, 1.0 - eps)
    ...
    w_vars = [var for var in t_vars if ("G" or "D") in var.name]

    # Regularization for weights
    gr_loss = tf.contrib.layers.apply_regularization(regularizer=tf.contrib.layers.l1_regularizer(1.0e-4),
                                                     weights_list=g_vars)
...
with tf.Session(graph=g) as sess:
    ...
    ckpt = tf.train.get_checkpoint_state('./model/GAN')
    if ckpt and ckpt.model_checkpoint_path:
        ckpt_name = os.path.basename(ckpt.model_checkpoint_path)
        saver.restore(sess, os.path.join('./model/GAN', ckpt_name))
    ...
    logPath = "./result/GAN/GAN_result.log"
    ...
    for epoch in range(counter, total_epochs):
        ...
        if (epoch + 1) % 5000 == 0 or epoch == 0:
            # save model
            modelPath = "./model/GAN/{0:s}_{1:s}_{2:s}".format(phones, freq, snr)
    ...
    for i in range(batch_size):
        savePath = './wave/GAN/{}.wav'.format(str(i).zfill(3))
