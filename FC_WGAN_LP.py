...
with g.as_default():
    ...
    extend = tf.ones(shape=[1, nLength], dtype=tf.float32)
    alphaM = tf.matmul(alpha, extend)
    interpolation = alphaM * X + (1. - alphaM) * fake_x
    result_of_interpolation = discriminator(interpolation, True)
    gradients = tf.gradients(result_of_interpolation, [interpolation])[0]
    gradients = tf.reduce_sum(tf.abs(gradients), reduction_indices=[1])
    gradients = tf.clip_by_value(gradients - 1, 0, np.infty) ** 2
    gradient_penalty = tf.reduce_mean(gradients)
...
with tf.Session(graph=g) as sess:
    ...
    ckpt = tf.train.get_checkpoint_state('./model/WGAN_LP')
    if ckpt and ckpt.model_checkpoint_path:
        ckpt_name = os.path.basename(ckpt.model_checkpoint_path)
        saver.restore(sess, os.path.join('./model/WGAN_LP', ckpt_name))
    ...
    logPath = "./result/WGAN_LP/GAN_result.log"
    ...
    for epoch in range(counter, total_epochs):
        ...
        if (epoch + 1) % 5000 == 0 or epoch == 0:
            # save model
            modelPath = "./model/WGAN_LP/{0:s}_{1:s}_{2:s}".format(phones, freq, snr)
    ...
    for i in range(batch_size):
        savePath = './wave/WGAN_LP/{}.wav'.format(str(i).zfill(3))
